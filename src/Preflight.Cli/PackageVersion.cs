...
/// parse rather than sorting somewhere surprising. See ADR-023 for the same
/// refusal applied to glob patterns and string ranges.
/// </para>
/// <para>
/// The rule in <c>Docs/design.md 11.2</c> that a patch difference never decides
/// compatibility governs <c>Preflight.Abstractions</c>, and nothing else. Here
/// <c>1.4.1</c> and <c>1.4.0</c> are two different versions. The two axes are
/// the false friend this phase creates, and neither may influence the other.
/// </para>
/// </remarks>
    ...
    : IComparable<PackageVersion>
{
    ...
    /// silence, which is the one outcome principle nº7 forbids.
    /// </remarks>
