...
/// the checkout, declared in its own base document; which version of it is on
/// this disk cannot live there, because one developer holds the clones of two
/// games on one machine and only one of those checkouts could be right about
/// what is installed.
/// </para>
/// <para>
/// The layout is <c>&lt;root&gt;/pipelines/&lt;name&gt;/&lt;version&gt;/</c>, with
/// the machine's own state beside it.
/// </para>
/// </remarks>
...
public sealed record PipelineInstallRoot(DirectoryInfo Root)
{
    ...
    /// already applies to the five variables it looks for — an
    /// exported-but-empty variable is a shell artefact, not an instruction.
    /// </para>
    /// <para>
    ...
    /// <c>cachePath</c> already makes, sharpened: the run would load rule
    /// assemblies out of a tree the person running <c>preflight</c> did not
    /// write. That is why the implicit <c>rules/</c> directory is probed beside
    /// the executable and never beside the workspace — a checkout under
    /// validation must not be able to supply the code that validates it.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// The same containment test <c>cachePath</c> uses, and for the same
    /// reason: a path that swallows the workspace turns a read of the
