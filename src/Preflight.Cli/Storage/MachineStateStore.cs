...
public sealed class MachineStateStore : IMachineStateStore
{
        ...
        MachineStateDocument document, string path)
    {
        ...
        foreach (var (name, text) in document.Pins ?? [])
        {
            ...
            // whichever the reader reached last. Refused for the reason two
            // policy target keys of equal specificity are refused: when two
            // spellings address one thing, picking a winner decides in silence
            // what nobody decided.
