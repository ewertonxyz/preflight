...
/// <summary>
/// The third of the three types that write, and the one with the opposite
/// promise to the first.
/// </summary>
/// <remarks>
/// <para>
...
/// what an idempotent <c>install</c> needs. The two are opposite on purpose —
/// one guards a file a person authored, the other holds a directory whose whole
/// job is to be replaced — and the test on each side is what makes a later
/// "unification" break loudly instead of quietly relaxing one of them.
/// <c>WorkspaceFileWriterTests</c> is the other half.
/// </para>
/// <para>
/// The staging directory lives inside the install root rather than in the
/// system temp, and the reason is <c>File.Move</c>: it is not atomic across
/// volumes, and <c>PREFLIGHT_HOME</c> pointing at another drive is an ordinary
/// thing for somebody to do. A half-moved version directory is an install that
