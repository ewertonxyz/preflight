...
public sealed class ExitCodeTests
{
    ...
    /// <see cref="RunVerdict"/> without visiting this table. Returning 0 for a
    /// verdict nothing mapped would report success over a run whose outcome this
    /// table never understood, which is the one failure this tool exists to
    /// prevent, so the switch throws rather than defaulting.
    /// </remarks>
    [Fact]
    ...
    /// <remarks>
    /// <para>
    /// The mapping catches the base and never a subtype, so nothing a subtype
    /// does can make this fail today. It is written anyway, and by reflection
    /// rather than by a hand-written list, because refusals keep arriving —
