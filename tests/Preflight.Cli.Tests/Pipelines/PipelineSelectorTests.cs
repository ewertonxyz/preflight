...
/// part that decides everything else, and what makes a file a candidate is
/// semantic rather than textual: it has to parse as a policy document. Deciding
/// by name alone was tried and refused by defect — <c>preflight.deps.json</c>
/// and <c>preflight.runtimeconfig.json</c> are emitted by the .NET build beside
/// the executable and match the pattern, so a name rule refuses to run in any
/// directory containing them, over pipelines that do not exist.
/// </remarks>
public sealed class PipelineSelectorTests : IDisposable
{
    ...
    private void Write(string name, string content) =>
        WorkspaceFiles.Write(_workspace, name, content);
    ...
    private PipelineSelection Select(string? explicitPipeline = null) =>
        PipelineSelector.Select(
            ...
            _fileSystem,
            explicitPipeline,
            CheckoutDocument.Read(_workspace, _fileSystem),
    ...
    /// <remarks>
    /// <c>production</c> is the old spelling of this root key and is still
    /// accepted, so the checkout selection has to read it too — otherwise a
    /// policy file written before the rename would parse, validate, and select
    /// nothing. Removing the spelling instead was refused because the policy
    /// schema is strict by decision: every file written before the rename would
    /// become a load error on the day of the upgrade.
    /// </remarks>
    [Fact]
    ...
    /// </summary>
    /// <remarks>
    /// Falling back to the base would validate against a policy nobody selected
    /// and report success, which is the worst thing a validation tool can do,
    /// reached here by a workspace that grew a second pipeline. The candidates
    /// are named, in ordinal order, because a refusal is worth more when it says
    /// what would have worked — and because a list ordered by the file system
    /// reads differently on two machines, which makes it a list nobody trusts.
    /// </remarks>
    [Fact]
    ...
    /// what it validates without a file being edited. The pipeline is declared,
    /// never inferred, and a single candidate is still an inference.
    /// </remarks>
    [Fact]
