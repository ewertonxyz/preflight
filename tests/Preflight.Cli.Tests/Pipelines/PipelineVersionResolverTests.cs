...
/// reading twice are a pin outside the checkout's range and a checkout that both
/// requires a package and carries its own policy file. Neither is allowed to
/// resolve to something workable: silently moving off a pin that no longer fits
/// would run a version the machine did not choose, and picking between the
/// package and the local file would configure the run from whichever one the
