...
/// output — a report that cannot tell them apart is a report that says nothing
/// about which policy actually ran. And a path in it must not send its reader
/// looking through the checkout for a file that lives somewhere else entirely,
/// which is why the package name and version come <em>before</em> the file
/// rather than after it: the qualification has to arrive before the reader
/// starts searching.
/// </remarks>
