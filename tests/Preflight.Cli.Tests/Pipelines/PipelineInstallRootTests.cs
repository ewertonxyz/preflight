...
/// <remarks>
/// <para>
/// The root is a fact about the machine and the pipeline selection is a fact
/// about the checkout, and they are kept apart on purpose: a developer with two
/// games cloned side by side would otherwise walk into the wrong directory,
/// validate against the other game's policy, and be told it passed.
/// </para>
/// <para>
/// Every branch is exercised through a substituted environment reader rather
/// than by setting variables on the process running the tests, because a test
/// that mutated <c>PREFLIGHT_HOME</c> would decide the result of whichever test
/// ran next to it.
/// </para>
/// </remarks>
public sealed class PipelineInstallRootTests : IDisposable
{
    ...
    /// <remarks>
    /// The same argument <c>cachePath</c> forced, sharpened by what an install
    /// root holds. A path in a policy file that resolves inside the workspace
    /// turns a validation tool into one that writes to the repository it is
    /// validating; here it would additionally mean rule assemblies loaded out of
    /// the tree under validation, which is the one thing the implicit
    /// <c>rules/</c> directory is resolved against the executable to avoid.
    /// </remarks>
    [Fact]
