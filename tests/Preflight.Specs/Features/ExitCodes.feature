Feature: Exit codes
    ...
    # The one row of 8.4 a pipeline is most likely to misread. System.CommandLine
    ...
    # The same trap, on the flag the reporters constrained. AcceptOnlyFromAmong on
    # --format is a new parse-failure path, and System.CommandLine returns 1 by
    # default for every one of them — so the documented override
    # has to cover this one too. A capital letter is the misspelling somebody
    # actually types, and until the reporters it produced a console screen and exit 0
    # for a caller that had asked for a machine.
    ...
    # cheapest way to provoke one, because 6.1 leaves settings uninspected on
    # purpose — the validator cannot catch it at load time.
